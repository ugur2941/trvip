    ...
    with sync_playwright() as p:
        ...
        for i, (channel_id, (channel_name, category)) in enumerate(channels.items(), 1):
            try:
                ...
                # 1. Yöntem: Esnek Regex ile baseurl arama (const, var, let kabul eder)
                match = re.search(r'(?:const|var|let)\s+baseurl\s*=\s*["\'](.*?)["\']', content)
                ...
                if match:
                    ...
                    # 2. Yöntem: Doğrudan m3u8 linki arama (Fallback)
                    m3u8_match = re.search(r'["\'](http[^"\']+\.m3u8)["\']', content)
                    if m3u8_match:
                        direct_url = m3u8_match.group(1)
                        print("-> ⚠️ BaseURL yok, doğrudan link bulundu.", end=" ")
                
                if not direct_url:
                    print("-> ❌ Link bulunamadı. Debug için HTML kaydediliyor.")
...
#EXT-X-ORIGIN:{domain}"""
            ...
            print("\nℹ️  BaseURL veya m3u8 linki içeren hiçbir kanal bulunamadı.")
